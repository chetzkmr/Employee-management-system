...
--CHANGING THE DEPARTMENTID COLUMN NAME--
ALTER TABLE Employee DROP CONSTRAINT FK__Employee__Depart__1BFD2C07;

EXEC sp_rename 'Employee.Department_ID', 'DepartmentID', 'COLUMN';
...
SELECT JobTitleName, COUNT(EmployeeID) AS Total_Number
FROM Employee
...
SELECT CONCAT(FirstName,' ', LastName) AS Full_Name, DateOfBirth, Gender, PhoneNumber, HireDate
FROM Employee
...
SELECT CONCAT(M.FirstName, ' ', M.LastName) AS Full_name, COUNT(E.EmployeeID) AS Total_count
FROM Employee E
INNER JOIN Employee M
ON E.ManagerID = M.EmployeeID
...
SELECT CONCAT(E.FIRSTNAME,' ', E.LASTNAME), D.DEPARTMENTNAME, S.BaseSalary + S.Bonus -S.Deductions AS Total_salary
FROM EMPLOYEE E
INNER JOIN Department D
ON E.DepartmentID = D.DepartmentID
INNER JOIN Salary S
